...
package net.mcreator.shardsoftheglobe.world.dimension;
...
import net.mcreator.shardsoftheglobe.procedures.TheendofspaceandtimePlayerEntersDimensionProcedure;
import net.mcreator.shardsoftheglobe.item.TheendofspaceandtimeItem;
...
public class TheendofspaceandtimeDimension extends ShardsOfTheGlobeModElements.ModElement {
	@ObjectHolder("shards_of_the_globe:theendofspaceandtime_portal")
	public static final CustomPortalBlock portal = null;
	public TheendofspaceandtimeDimension(ShardsOfTheGlobeModElements instance) {
		super(instance, 6);
	...
	public void clientLoad(FMLClientSetupEvent event) {
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("shards_of_the_globe:theendofspaceandtime"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("theendofspaceandtime_portal", Vector3i::compareTo, 300);
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("theendofspaceandtime_portal",
					Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1)
							.setRegistryName("theendofspaceandtime_portal");
	...
	public void initElements() {
		...
		elements.items.add(() -> new TheendofspaceandtimeItem().setRegistryName("theendofspaceandtime"));
	}
			...
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote && true) {
				if (entity.func_242280_ah()) {
					entity.func_242279_ag();
						...
						new ResourceLocation("shards_of_the_globe:theendofspaceandtime"))) {
					entity.func_242279_ag();
					teleportToDimension(entity, pos,
							RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("shards_of_the_globe:theendofspaceandtime")));
				} else {
					entity.func_242279_ag();
	...
	public static class CustomPortalSize {
		private static final AbstractBlock.IPositionPredicate POSITION_PREDICATE = (state, blockReader, pos) -> {
			return state.getBlock() == Blocks.POLISHED_BLACKSTONE.getDefaultState().getBlock();
		};
					...
					for (int k2 = 0; k2 < 2; ++k2) {
						for (int i3 = -1; i3 < 3; ++i3) {
							BlockState blockstate1 = i3 < 0
									? Blocks.POLISHED_BLACKSTONE.getDefaultState().getBlock().getDefaultState()
									: Blocks.AIR.getDefaultState();
			...
			for (int k1 = -1; k1 < 3; ++k1) {
				for (int i2 = -1; i2 < 4; ++i2) {
					if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, Blocks.POLISHED_BLACKSTONE.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
	...
	public void onPlayerChangedDimensionEvent(PlayerEvent.PlayerChangedDimensionEvent event) {
		...
		if (event.getTo() == RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("shards_of_the_globe:theendofspaceandtime"))) {
			{
				Map<String, Object> $_dependencies = new HashMap<>();
				$_dependencies.put("entity", entity);
				TheendofspaceandtimePlayerEntersDimensionProcedure.executeProcedure($_dependencies);
			}
		}
	}
}
